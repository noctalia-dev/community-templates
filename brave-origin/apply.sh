...
# Never writes Brave Origin Preferences while Brave Origin is running.
set -euo pipefail
IFS=$'\n\t'

log() {
    printf '[%s] noctalia-brave-origin: %s\n' "$(date '+%Y-%m-%d %H:%M:%S')" "$*" >&2
}
...
brave_is_running() {
    pgrep -u "$(id -u)" -x brave-origin >/dev/null 2>&1
}
...
brave_user_data="${xdg_config}/BraveSoftware/Brave-Origin"
...
theme_dir="${xdg_data}/noctalia/brave-origin-theme"
...
rendered_manifest="${xdg_cache}/noctalia/brave-origin-theme/manifest.json"
...
ensure_brave_profile() {
    if [ ! -d "$brave_user_data" ]; then
        log "ERROR: Brave Origin user data not found: $brave_user_data"
        exit 1
    fi
    if [ ! -f "$prefs_path" ]; then
        log "ERROR: Brave Origin Preferences not found: $prefs_path"
        exit 1
    fi
}
...
backup_prefs() {
    ...
    if [ -f "$prefs_original" ]; then
        ...
        if brave_is_running; then
            log "NOTE: Brave Origin is running; this is an on-disk snapshot and may lag in-memory prefs"
        fi
    fi
...
print_load_unpacked_help() {
    cat <<EOF >&2
noctalia-brave-origin: first-time install (click through in Brave Origin):
...
sync_color_scheme() {
    ...
    if brave_is_running; then
        log "Brave Origin is running: skipped Preferences write"
        log "Settings checkboxes/buttons use Brave Leo purple, not the theme pack"
        log "Quit Brave Origin completely, then re-run: $0 $mode"
...
restore_prefs() {
    ...
    if brave_is_running; then
        log "ERROR: Brave Origin is running; will not restore Preferences"
        exit 1
    fi
